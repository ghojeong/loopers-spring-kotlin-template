...
- **기능 흐름**
  ...
  - 3. 해당 사용자의 좋아요 기록이 이미 존재하는지 확인한다 (비관적 락 사용).
...
- **도메인 객체 책임**
  ...
  - `LikeService (Domain Service)`: 중복 확인 로직 (비관적 락 사용), 멱등성 보장
  - `LikeRepository (Interface in Domain)`: 좋아요 저장/조회/삭제 인터페이스 (`findByUserIdAndProductIdWithLock()` 포함)
