...
- **기능 흐름**
  - 1. X-USER-ID 헤더로 사용자를 식별한다.
  - 2. 상품 ID로 상품의 존재 여부를 확인한다.
  - 3. 좋아요 삭제를 시도하고 삭제된 행 수를 반환받는다.
  - 4. 실제로 삭제된 경우에만 Redis에서 좋아요 수를 감소시킨다.
  - 5. 실제로 삭제된 경우에만 캐시를 무효화한다.
  - 6. 삭제된 행이 없으면 아무 동작도 하지 않는다 (멱등성).
- **제약사항**
  - 멱등성 보장: 동일 요청 반복 시 에러 없음
  - 존재하지 않는 상품에 대한 취소 요청 시 적절한 처리
  - 삭제된 행 수 확인을 통한 정확한 Redis 카운트 관리
- **도메인 객체 책임**
  - `LikeService (Domain Service)`: 삭제 후 행 수 확인, Redis 감소 및 캐시 무효화 제어, 멱등성 보장
  - `LikeRepository (Interface in Domain)`: 좋아요 삭제 및 삭제된 행 수 반환 (`Long` 타입)
