...
고민 끝에 낙관적 락을 택했다. 이유는 단순했다. **"대부분의 경우 충돌이 없을 것 같았으니까."**

같은 상품을 정확히 같은 순간에 여러 명이 주문하는 경우가 얼마나 될까? 블랙 프라이데이나 한정판 이벤트가 아니라면, 충돌은 드물 거다. 그리고 설령 충돌이 나도 2~3번 재시도하면 성공할 확률이 높다.
...
class Stock(
    val productId: Long,
    var quantity: Int,
    @Version var version: Int = 0  // JPA가 자동으로 충돌 감지
)
```

ERD에도 version 컬럼을 추가했다. 업데이트할 때 version이 다르면 실패하고, 재시도하는 방식이다.

물론 완벽한 선택은 아니다. 정말 트래픽이 몰리는 상품이라면 비관적 락이나 Redis 분산 락을 고려해야 할 수도 있다. 하지만 지금 단계에서는 낙관적 락이 **"합리적인 선택"**이라고 판단했다.

### 배운 것

동시성 제어는 정답이 없더라. "무조건 이 방법이 좋다"가 아니라 **"이 시스템에서는 이게 더 적합하다"**를 고민해야 했다.

처음엔 "락 하나 걸면 되는 거 아냐?"라고 생각했는데, 락의 종류마다 트레이드오프가 있다는 걸 배웠다. 성능을 택할지, 안정성을 택할지. 그 판단 기준이 결국 "예상되는 충돌 빈도"였다.
...
- `order_items`에 `price_at_order`를 저장하는 게 맞나? 상품 가격이 바뀌어도 주문 이력은 유지해야 하니까 넣었는데, 데이터 중복이긴 하다.
